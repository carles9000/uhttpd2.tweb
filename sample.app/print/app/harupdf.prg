...
FUNCTION DesignHaruPDF( oDom, cFileToSave )
   ...
   IF pdf == NIL
      oDom:SetMsg( "PDF could not be created!" )
      RETURN NIL
   ENDIF
   ...
   */
   ...
   FOR i := 1 TO Len( font_list )
      //samp_text := "abcdefgABCDEFG12345!#$%&+-@?àáèéíòóúçñ"
   ...
   IF HPDF_SaveToFile( pdf, cFileToSave ) != 0
      _d( "0x" + hb_NumToHex( HPDF_GetError( pdf ), 4 ), hb_HPDF_GetErrorString( HPDF_GetError( pdf ) ), HPDF_GetErrorDetail( pdf ) )
   ENDIF
...
STATIC PROCEDURE Page_Images( pdf )
   ...
   LOCAL cImagePath := hb_DirBase() + "files\images\" 
